import{S as e}from"./QueenCombBabylon-DOdGX8_Q.js";import"./mainUVVaryingDeclaration-CT8wytya.js";import"./oitFragment-BhKa8NRe.js";const t="pbrFragmentExtraDeclaration",g=`varying vec3 vPositionW;
#if DEBUGMODE>0
...
#endif
`;e.IncludesShadersStore[t]||(e.IncludesShadersStore[t]=g);const M={name:t,shader:g},i="subSurfaceScatteringFunctions",u=`bool testLightingForSSS(float diffusionProfile)
{return diffusionProfile<1.;}`;e.IncludesShadersStore[i]||(e.IncludesShadersStore[i]=u);const F={name:i,shader:u},o="importanceSampling",h=`vec3 hemisphereCosSample(vec2 u) {float phi=2.*PI*u.x;float cosTheta2=1.-u.y;float cosTheta=sqrt(cosTheta2);float sinTheta=sqrt(1.-cosTheta2);return vec3(sinTheta*cos(phi),sinTheta*sin(phi),cosTheta);}
...
float phi=2.*PI*u.x;float sinTheta=pow(u.y,a/(2.*a+1.));float cosTheta=sqrt(1.-sinTheta*sinTheta);return vec3(sinTheta*cos(phi),sinTheta*sin(phi),cosTheta);}`;e.IncludesShadersStore[o]||(e.IncludesShadersStore[o]=h);const O={name:o,shader:h},a="pbrHelperFunctions",E=`#define MINIMUMVARIANCE 0.0005
...
#endif
`;e.IncludesShadersStore[a]||(e.IncludesShadersStore[a]=E);const _={name:a,shader:E},n="pbrDirectLightingSetupFunctions",L=`struct preLightingInfo
...
result.areaLightDiffuse=data.Diffuse;result.LdotV=0.;result.roughness=0.;result.diffuseRoughness=0.;result.surfaceAlbedo=vec3(0.);return result;}
#endif
`;e.IncludesShadersStore[n]||(e.IncludesShadersStore[n]=L);const I={name:n,shader:L},r="pbrDirectLightingFalloffFunctions",p=`float computeDistanceLightFalloff_Standard(vec3 lightOffset,float range)
...
#endif
}`;e.IncludesShadersStore[r]||(e.IncludesShadersStore[r]=p);const R={name:r,shader:p},l="hdrFilteringFunctions",D=`#if NUM_SAMPLES
#if NUM_SAMPLES>0
...
#endif
#endif
`;e.IncludesShadersStore[l]||(e.IncludesShadersStore[l]=D);const B={name:l,shader:D},f="pbrDirectLightingFunctions",v=`#define CLEARCOATREFLECTANCE90 1.0
struct lightingInfo
{vec3 diffuse;
...
#endif
`;e.IncludesShadersStore[f]||(e.IncludesShadersStore[f]=v);const b={name:f,shader:v},c="pbrBlockNormalGeometric",m=`vec3 viewDirectionW=normalize(vEyePosition.xyz-vPositionW);
#ifdef NORMAL
...
#endif
`;e.IncludesShadersStore[c]||(e.IncludesShadersStore[c]=m);const U={name:c,shader:m},s="pbrBlockImageProcessing",A=`#if defined(IMAGEPROCESSINGPOSTPROCESS) || defined(SS_SCATTERING)
...
#endif
`;e.IncludesShadersStore[s]||(e.IncludesShadersStore[s]=A);const G={name:s,shader:A},d="pbrDebug",C=`#if DEBUGMODE>0
...
#endif
`;e.IncludesShadersStore[d]||(e.IncludesShadersStore[d]=C);const P={name:d,shader:C};export{_ as a,I as b,R as c,b as d,U as e,G as f,P as g,B as h,O as i,M as p,F as s};
