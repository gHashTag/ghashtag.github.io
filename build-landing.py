...
PAGES = {
    ...
    "about": {
        ...
        "sections": [
            ...
            ("Shipped", [
                ("Orb — screen recorder for macOS", "A camera bubble that floats above every other app and is never welded into the video. Screen, camera and pointer stay separate tracks until export, so the bubble can be moved, resized or reshaped after the take is over — and the glass rim refracts the screen behind it, because those pixels are still there. Native Swift on ScreenCaptureKit, AVFoundation and Core Image. The product page is at t27.ai/orb."),
            ]),
            ("Working with me", [
...
def leela_surfaces():
    """The three Leela surfaces, which live in a different Pages site.

    `t27.ai/leela/` is `gHashTag/leela`'s project site, served under this domain
    because this repository owns the CNAME. That is why these three cannot be
    read off disk the way `doc_pages` and `blog_pages` are: the files are not
    here, and this generator has no way to see them.

    **Measured 2026-08-31: the word "leela" appeared nowhere in this repository
    at all.** A game, the 2D board it replaced, and a 1,784-page book in
    twenty-two languages were live on this domain and in none of its 182
    sitemap URLs — reachable only from a Telegram button. The same shape as the
    forty-eight chapters `doc_pages` was written for, one site over.

    A hand-kept list, which this file argues against everywhere else, and the
    argument does not apply: these three are not a growing set of documents but
    the fixed shape of the other repository's artifact, pinned by its
    `pages.yml` — the 3D board at the root, `classic/` beside it, `docs/` under
    it. The book's own chapters are deliberately NOT listed: there are 1,784 of
    them, this generator cannot see one, and listing them would be inventing
    URLs rather than publishing them.
...
def sitemap(slugs):
    ...
    # /orb/ is served by gHashTag/orb's own Pages site, not from this repository —
    # a project site inherits the custom domain from the user site that owns the
    # CNAME. So it is listed here for exactly the reason leela_surfaces() exists:
    # the files are not here, this generator cannot see them, and a page absent
    # from the sitemap is a page that does not exist.
    extra = ["status/", "orb/"] + [f"r/{s}/" for s in result_slugs()]
    ...
    paths = [""] + [f"{x}/" for x in slugs] + extra + blog_pages() + doc_pages() + leela_surfaces()
    out = []
    for s in paths:
        lm = _lastmod(s)
